...
class CameraControllerApp:
    ...
    def run_ssh_command(self, command):
        ...
        try:
            if SSH_KEY_PATH:
                ssh.connect(SSH_HOST, username=SSH_USER, key_filename=SSH_KEY_PATH, timeout=5)
            else:
    ...
    def load_configs(self):
        if not os.path.exists("camera_config.yaml"):
            messagebox.showerror("Error", "camera_config.yaml not found. Run scanner.py first.")
            self.root.destroy()
            return
    ...
    def build_gui(self):
        for cam_name, cam_data in self.config.items():
            ...
            ttk.Button(header, text="Check Path Exists", command=lambda n=cam_name: self.check_path(n)).pack(side='left', padx=5)
            ...
            ttk.Button(header, text="Get Settings", command=lambda n=cam_name: self.get_settings(n)).pack(side='right', padx=5)
            ...
            for ctrl_name, props in cam_data['controls'].items():
                ...
                if props['type'] in ['int', 'menu']:
                    ...
                    scale = ttk.Scale(row, from_=min_val, to=max_val, variable=var, orient='horizontal')
                    ...
                    if props['type'] == 'menu':
                        ...
                        ttk.Label(row, textvariable=menu_var, width=25, foreground="blue").pack(side='left', padx=5)
            ...
            ttk.Button(file_frame, text="Load Settings", command=lambda n=cam_name: self.load_settings(n)).pack(side='left', padx=5)
            ttk.Button(file_frame, text="Save Settings", command=lambda n=cam_name: self.save_settings(n)).pack(side='left', padx=5)
            
            ttk.Button(footer, text="Set Settings", command=lambda n=cam_name: self.set_settings(n)).pack(side='right', padx=5)
    ...
    def get_settings(self, cam_name):
        ...
        pattern = re.compile(r"^\s*([a-zA-Z0-9_]+)\s+.*value=(-?\d+)(?:\s+\(([^)]+)\))?")
            ...
            if match:
                ...
                if ctrl_name in self.tabs[cam_name]['vars']:
                    ...
                    if ctrl_name in self.tabs[cam_name]['menu_vars'] and menu_text:
                        self.tabs[cam_name]['menu_vars'][ctrl_name].set(f"({menu_text})")
    ...
    def load_settings(self, cam_name):
        ...
        try:
            ...
            print(f"Loaded {filename} into GUI. Click 'Set Settings' to push these changes to the camera.")
        except Exception as e:
    ...
    def set_settings(self, cam_name):
        ...
        sorted_ctrl_names = sorted(
            changes.keys(),
            key=lambda k: 0 if 'auto' in k.lower() else 1
        )
