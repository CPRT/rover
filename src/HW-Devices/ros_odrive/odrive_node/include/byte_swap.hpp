...
template <std::endian endianness, typename T> T maybe_byteswap(T val) {
...
template <typename T> T read_le(const unsigned char *buf) {
  return maybe_byteswap<std::endian::little>(*(T *)buf);
}

template <typename T> void write_le(const T &val, const unsigned char *buf) {
