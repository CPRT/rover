...
bool SocketCanIntf::init(const std::string &interface,
  ...
  if (bind(socket_id_, reinterpret_cast<struct sockaddr *>(&addr),
           sizeof(addr)) == -1) {
  ...
  struct msghdr message = {.msg_name = nullptr,
                           .msg_namelen = 0,
                           .msg_iov = nullptr,
                           .msg_iovlen = 0,
                           .msg_control = nullptr,
  ...
  if (!event_loop_->register_event(
          &socket_evt_id_, socket_id_, EPOLLIN,
