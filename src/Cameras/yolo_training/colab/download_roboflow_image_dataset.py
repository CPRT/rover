...
def roboflow_download_func(
    datasetUrl: str, modelFormat: str, outputDir: str, api_key: str
):
...
def download_roboflow_dataset(
    url: str, dataset_version: int, model_format: str, output_dir: str, api_key: str
) -> bool:
    """
...
def download_datasets_from_config(
    config_path: str, model_format: str, base_output_dir: str, api_key: str
):
    ...
    for i, dataset in enumerate(datasets, 1):
        ...
        try:
            download_roboflow_dataset(
                url, dataset_version, model_format, output_dir, api_key
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Download datasets from Roboflow using a config file"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="yolov9",
        help="Model format (default: yolov9)",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    if not api_key:
        parser.error(
            "API key must be provided via --api-key argument or ROBOFLOW_API_KEY environment variable"
        )
    ...
    download_datasets_from_config(
        args.config, args.model_format, args.output_dir, api_key
    )
