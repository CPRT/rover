...
def parse_split(split_str: str) -> Tuple[float, float, float]:
    ...
    if len(parts) != 3:
        raise ValueError(
            f"Invalid split format: {split_str}. Expected format: XX_YY_ZZ"
        )
...
def merge_class_names(
    all_class_names: List[List[str]], label_mapping: Dict[str, int]
) -> Tuple[List[str], Dict[int, Dict[int, int]]]:
    """
    ...
    for dataset_idx, names in enumerate(all_class_names):
        ...
        for old_id, old_name in enumerate(names):
            if old_name in label_mapping:
                ...
                print(
                    f"Warning: Class '{old_name}' in dataset {dataset_idx} not found in label_mapping"
                )
...
def main():
    ...
    print(
        f"Using split ratios - Train: {split_ratios[0]:.0%}, Val: {split_ratios[1]:.0%}, Test: {split_ratios[2]:.0%}"
    )
    ...
    generate_dataset(
        all_pairs, output_path, split_ratios, merged_classes, class_mappings
    )
