...
def estimate_pose_single_markers(corners, marker_size, camera_matrix, dist_coeffs):
    """
    Estimate the pose of single ArUco markers compatible with the current OpenCV version.

    Args:
        corners: Array of marker corners (for a single marker or list of markers)
        marker_size: Size of the marker in meters
        camera_matrix: Camera intrinsic matrix
        dist_coeffs: Camera distortion coefficients

    Returns:
        Tuple of (rvecs, tvecs, obj_points) where:
        - rvecs: Rotation vectors for each marker
        - tvecs: Translation vectors for each marker
        - obj_points: Object points used (or None in new API)
    """
    if IS_NEW_API:
        # New API: Use cv2.solvePnP for each marker
        # Define the 3D coordinates of marker corners in marker frame
        ...
        # Handle both single marker and multiple markers
        corners_array = corners if isinstance(corners, (list, tuple)) else [corners]

        for corner in corners_array:
            # corner is shape (1, 4, 2) or (4, 2)
            if corner.ndim == 3:
                corner_2d = corner[0]
            else:
                corner_2d = corner
