...
def launch_setup(context, *args, **kwargs):
    ...
    info = (
        "Using Object Detection configuration file: "
    ...
    info = (
        "Using Custom Object Detection configuration file: "
    ...
    if enable_ipc_effective and not nitros_disabled_effective:
        ...
        if nitros_installed:
            ...
            if ipc_nitros_conflict_policy_val == "disable_ipc":
                ...
                return_array.append(
                    LogInfo(
                        msg=TextSubstitution(
                            text="WARNING: "
                            + conflict_msg
                            + " Forcing `enable_ipc:=false` for this launch."
                        )
                    )
                )
            else:
                raise RuntimeError(
                    conflict_msg
                    + " Set `enable_ipc:=false` or `debug.disable_nitros:=true`."
                )
        else:
            return_array.append(
                LogInfo(
