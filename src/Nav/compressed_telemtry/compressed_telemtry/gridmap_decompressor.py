...
class GridMapDecompressor(Node):
    ...
    def __init__(self):
        ...
        self.pub = self.create_publisher(
            GridMap, '/viz/traversability_map', rviz_qos)
            
        self.sub = self.create_subscription(
            UInt8MultiArray, '/telemetry/gridmap_compressed', self.map_callback, radio_qos)
    ...
    def map_callback(self, msg):
        try:
            ...
            if len(raw_payload) < struct_size:
                self.get_logger().warning('Dropped gridmap packet: payload smaller than header.')
                return
            ...
            px, py, pz, qx, qy, qz, qw = meta[5], meta[6], meta[7], meta[8], meta[9], meta[10], meta[11]
            ...
            if len(raw_payload) < frame_end:
                self.get_logger().warning('Dropped gridmap packet: truncated frame_id bytes.')
                return
            ...
            if len(remaining_bytes) % 3 != 0:
                self.get_logger().warning('Dropped gridmap packet: payload alignment is invalid.')
                return
            ...
            elev_float = np.where(elev_int16 == -32768, np.nan, elev_int16 / 1000.0).astype(np.float32)
            trav_float = np.where(trav_uint8 == 255, np.nan, trav_uint8.astype(np.float32) / 100.0)
