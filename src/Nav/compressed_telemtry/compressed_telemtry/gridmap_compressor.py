...
class GridMapCompressor(Node):
    ...
    def map_callback(self, msg):
        try:
            ...
            elev_int16[elev_valid] = np.clip(elev_arr[elev_valid] * 1000.0, -32767, 32767).astype(np.int16)
            ...
            trav_uint8[trav_valid] = np.clip(trav_arr[trav_valid] * 100.0, 0, 254).astype(np.uint8)
            ...
            metadata = struct.pack('<2B 10d 2H',
                self.PROTOCOL_VERSION,
                len(frame_bytes),
                msg.info.resolution, msg.info.length_x, msg.info.length_y,
                msg.info.pose.position.x, msg.info.pose.position.y, msg.info.pose.position.z,
                msg.info.pose.orientation.x, msg.info.pose.orientation.y, msg.info.pose.orientation.z, msg.info.pose.orientation.w,
                msg.outer_start_index, msg.inner_start_index
            )
            
            # 6. Assemble the binary payload and Compress
            raw_payload = metadata + frame_bytes + elev_int16.tobytes() + trav_uint8.tobytes()
