...
MultiDOFJointTrajectoryPointConnectionVisual::
  ...
  assert(from.transforms.size() == to.transforms.size() &&
         "ERROR: MultiDOFJointTrajectoryPointConnectionVisual: from and to "
  ...
  for (unsigned int i = 0; i < from.transforms.size(); i++) {
    lines_.push_back(boost::shared_ptr<rviz::Line>(
        new rviz::Line(scene_manager_, scene_node_)));
    const Ogre::Vector3 fromVector(from.transforms[i].translation.x,
                                   from.transforms[i].translation.y,
                                   from.transforms[i].translation.z);
    const Ogre::Vector3 toVector(to.transforms[i].translation.x,
                                 to.transforms[i].translation.y,
                                 to.transforms[i].translation.z);
...
void MultiDOFJointTrajectoryPointConnectionVisual::setShowConnection(
    bool visible) {
