...
MultiDOFJointTrajectoryPointVisual::MultiDOFJointTrajectoryPointVisual(
  ...
  assert((msg.velocities.size() == captions.size() ||
          msg.velocities.size() == 0) &&
         "ERROR: MultiDOFJointTrajectoryPointVisual: velocity vector has "
         "invalid length.");
  assert((msg.accelerations.size() == captions.size() ||
          msg.accelerations.size() == 0) &&
         "ERROR: MultiDOFJointTrajectoryPointVisual: acceleration vector has "
         "invalid length.");

  const bool message_contains_velocities =
      (msg.velocities.size() == msg.transforms.size());
  const bool message_contains_accelerations =
      (msg.accelerations.size() == msg.transforms.size());

  // loop through all joints
  for (unsigned int i = 0; i < msg.transforms.size(); i++) {
    // transform position
    transforms_position_.push_back(parent_node->createChildSceneNode());
    transforms_position_.back()->setPosition(msg.transforms[i].translation.x,
                                             msg.transforms[i].translation.y,
                                             msg.transforms[i].translation.z);

    // transform rotation
    const Ogre::Quaternion orientation(
        msg.transforms[i].rotation.w, msg.transforms[i].rotation.x,
        msg.transforms[i].rotation.y, msg.transforms[i].rotation.z);
    transforms_rotation_.push_back(boost::shared_ptr<rviz::Axes>(new rviz::Axes(
        scene_manager_, transforms_position_.back(), size_transform_rotation,
        axis_radius_per_size_ * size_transform_rotation)));
    ...
    if (message_contains_velocities) {
      // velocity linear
      vector =
          Ogre::Vector3(msg.velocities[i].linear.x, msg.velocities[i].linear.y,
                        msg.velocities[i].linear.z);
      velocities_linear_absolute_.push_back(vector.length());
      velocities_linear_.push_back(
          boost::shared_ptr<rviz::Arrow>(new rviz::Arrow(
              scene_manager, transforms_position_.back(),
      ...
      // velocity angular
      vector = Ogre::Vector3(msg.velocities[i].angular.x,
                             msg.velocities[i].angular.y,
                             msg.velocities[i].angular.z);
      velocities_angular_absolute_.push_back(vector.length());
      velocities_angular_.push_back(boost::shared_ptr<rviz::Arrow>(
          new rviz::Arrow(scene_manager, transforms_position_.back(),
                          0.8 * scale_velocity_angular_ *
                              velocities_angular_absolute_.back(),
                          diameter_arrows_,
                          0.2 * scale_velocity_angular_ *
                              velocities_angular_absolute_.back(),
                          2.0 * diameter_arrows_)));
    ...
    if (message_contains_accelerations) {
      // acceleration linear
      vector = Ogre::Vector3(msg.accelerations[i].linear.x,
                             msg.accelerations[i].linear.y,
                             msg.accelerations[i].linear.z);
      accelerations_linear_absolute_.push_back(vector.length());
      accelerations_linear_.push_back(boost::shared_ptr<rviz::Arrow>(
          new rviz::Arrow(scene_manager, transforms_position_.back(),
                          0.8 * scale_acceleration_linear_ *
                              accelerations_linear_absolute_.back(),
                          diameter_arrows_,
                          0.2 * scale_acceleration_linear_ *
                              accelerations_linear_absolute_.back(),
                          2.0 * diameter_arrows_)));
      accelerations_linear_.back()->setDirection(vector);

      // acceleration angular
      vector = Ogre::Vector3(msg.accelerations[i].angular.x,
                             msg.accelerations[i].angular.y,
                             msg.accelerations[i].angular.z);
      accelerations_angular_absolute_.push_back(vector.length());
      accelerations_angular_.push_back(boost::shared_ptr<rviz::Arrow>(
          new rviz::Arrow(scene_manager, transforms_position_.back(),
                          0.8 * scale_acceleration_angular_ *
                              accelerations_angular_absolute_.back(),
                          diameter_arrows_,
                          0.2 * scale_acceleration_angular_ *
                              accelerations_angular_absolute_.back(),
                          2.0 * diameter_arrows_)));
    ...
    // text
    auto text = boost::shared_ptr<rviz::MovableText>(
        new rviz::MovableText(captions_[i]));
    text->setCharacterHeight(getCharacterHeight());
    texts_.push_back(text);
    texts_.back()->setTextAlignment(rviz::MovableText::H_CENTER,
...
void MultiDOFJointTrajectoryPointVisual::setShowTransformRotation(
    bool visible) {
...
void MultiDOFJointTrajectoryPointVisual::setShowAccelerationLinear(
    bool visible) {
  show_acceleration_linear_ = visible;
  updateColorAccelerationLinear();
}

void MultiDOFJointTrajectoryPointVisual::setShowAccelerationAngular(
    bool visible) {
...
void MultiDOFJointTrajectoryPointVisual::setScaleAccelerationLinear(
    float scale) {
  scale_acceleration_linear_ = scale;
  updateScaleAccelerationLinear();
}

void MultiDOFJointTrajectoryPointVisual::setScaleAccelerationAngular(
    float scale) {
  scale_acceleration_angular_ = scale;
  updateScaleAccelerationAngular();
}

void MultiDOFJointTrajectoryPointVisual::setAlphaTransformRotation(
    float alpha) {
...
void MultiDOFJointTrajectoryPointVisual::updateColorVelocityLinear() {
  for (unsigned int i = 0; i < velocities_linear_.size(); i++) {
    velocities_linear_[i]->setColor(
        getColor(color_velocity_linear_, show_velocity_linear_));
  }
}

void MultiDOFJointTrajectoryPointVisual::updateColorVelocityAngular() {
  for (unsigned int i = 0; i < velocities_angular_.size(); i++) {
    velocities_angular_[i]->setColor(
        getColor(color_velocity_angular_, show_velocity_angular_));
  }
}

void MultiDOFJointTrajectoryPointVisual::updateColorAccelerationLinear() {
  for (unsigned int i = 0; i < accelerations_linear_.size(); i++) {
    accelerations_linear_[i]->setColor(
        getColor(color_acceleration_linear_, show_acceleration_linear_));
  }
}

void MultiDOFJointTrajectoryPointVisual::updateColorAccelerationAngular() {
  for (unsigned int i = 0; i < accelerations_angular_.size(); i++) {
    accelerations_angular_[i]->setColor(
        getColor(color_acceleration_angular_, show_acceleration_angular_));
  }
}
