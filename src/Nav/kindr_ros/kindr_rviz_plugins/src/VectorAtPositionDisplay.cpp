...
VectorAtPositionDisplay::VectorAtPositionDisplay()
  ...
  length_scale_property_ = new rviz::FloatProperty(
      "Length scale", 1.0, "Scale of the length of the vector.", this,
      SLOT(updateScale()));

  width_scale_property_ = new rviz::FloatProperty(
      "Width scale", 1.0, "Scale of the width of the vector.", this,
      SLOT(updateScale()));
  width_scale_property_->setMin(0);

  show_text_property_ = new rviz::BoolProperty(
      "Show text", true, "Enable or disable text rendering.", this,
      SLOT(updateShowText()));

  color_property_ = new rviz::ColorProperty(
      "Color", QColor(0, 0, 0),
      "Color to draw the vector (if not defined by vector type).", this,
...
void VectorAtPositionDisplay::updateVectorAtPosition() {
  ...
  if (current_vector_at_position_->position_frame_id.empty() ||
      current_vector_at_position_->position_frame_id ==
          current_vector_at_position_->header.frame_id) {
    ...
    if (!context_->getFrameManager()->getTransform(
      ...
      ROS_ERROR("Error transforming from frame '%s' to frame '%s'",
                current_vector_at_position_->position_frame_id.c_str(),
    ...
    if (!context_->getFrameManager()->getTransform(
            current_vector_at_position_->header.frame_id,
            current_vector_at_position_->header.stamp, dummyPosition,
            orientationArrowFrameToFixedFrame)) {
      ROS_ERROR("Error transforming from frame '%s' to frame '%s'",
                current_vector_at_position_->header.frame_id.c_str(),
                qPrintable(fixed_frame_));
      return;
    }
  }
  ...
  if (visuals_.full()) {
    ...
    visual.reset(
        new VectorAtPositionVisual(context_->getSceneManager(), scene_node_));
  }
...
PLUGINLIB_EXPORT_CLASS(kindr_rviz_plugins::VectorAtPositionDisplay,
                       rviz::Display)
