...
# Prompt for writing the aruco localizer node
Can you now write a python node called aruco_board_localizer_node that will subscribe to the aruco board topic and will use the boards to produce a geometry_msgs/msg/PoseWithCovarianceStamped estimate of where the robot is in the map frame. It will do this by creating a known location for every board the first time it sees it. It should subscribe to the location of the rover in the map frame using the topic called /odometry/filtered/global which is a nav_msgs/msg/Odometry message in the map frame and then use the location of the rover plus the ArucoBoard message to calculate where the board is and store it. The tf trees are setup to properly define map -> odom -> base_link -> zed_camera_link so you can take advantage of that. Once a board is stored, its location is not changed and now when the board is seen this node should produce an estimate of where the rover using the ArucoBoard's pose in the camera frame and the known location of the board, and publish this as the geometry_msgs/msg/PoseWithCovarianceStamped. When a board is first seen, it shouldn't immediatly be stored in the known boards array, instead it should have samples be stored in a new_boards array and when there are at least 10 samples within a 2.0 second window that estimate the boards location within 0.3 meters then the board should be moved to the known_boards array. Make sure to use ros parameters so there's no magic numbers, and define good values for them in a config file, and make a launch file that has a launch argument to specify which config file to use. 