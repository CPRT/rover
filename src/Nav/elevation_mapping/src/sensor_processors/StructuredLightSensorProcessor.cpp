 ...
 * standardDeviationInNormalDirection = sensorModelNormalFactorA_ +
 * sensorModelNormalFactorB_ * (measurementDistance -
 * sensorModelNormalFactorC_)^2; standardDeviationInLateralDirection =
 * sensorModelLateralFactor_ * measurementDistance Taken from: Nguyen, C. V.,
 * Izadi, S., & Lovell, D., Modeling Kinect Sensor Noise for Improved 3D
 * Reconstruction and Tracking, 2012.
 */
...
bool StructuredLightSensorProcessor::readParameters(
  ...
  // nodeHandle_.param("sensor_processor/normal_factor_b",
  // sensorParameters_["normal_factor_b"], 0.0);
  // nodeHandle_.param("sensor_processor/normal_factor_c",
  // sensorParameters_["normal_factor_c"], 0.0);
  // nodeHandle_.param("sensor_processor/normal_factor_d",
  // sensorParameters_["normal_factor_d"], 0.0);
  // nodeHandle_.param("sensor_processor/normal_factor_e",
  // sensorParameters_["normal_factor_e"], 0.0);
...
bool StructuredLightSensorProcessor::computeVariances(
  ...
  const Eigen::Matrix3f rotationVariance =
      robotPoseCovariance.bottomRightCorner(3, 3).cast<float>();

  // Preparations for robot rotation Jacobian (J_q) to minimize computation for
  // every point in point cloud.
  ...
  for (unsigned int i = 0; i < pointCloud->size(); ++i) {
    ...
    const float deviationNormal =
        ...
        sensorParameters_.at("normal_factor_b") *
            ...
            (measurementDistance - sensorParameters_.at("normal_factor_c")) +
        sensorParameters_.at("normal_factor_d") *
            pow(measurementDistance, sensorParameters_.at("normal_factor_e"));
    const float varianceNormal = deviationNormal * deviationNormal;
    const float deviationLateral =
        sensorParameters_.at("lateral_factor") * measurementDistance;
    ...
    const Eigen::RowVector3f rotationJacobian =
    ...
    heightVariance += static_cast<float>(sensorJacobian * sensorVariance *
                                         sensorJacobian.transpose()) /
                      (epsilon + confidenceRatio * confidenceRatio);
