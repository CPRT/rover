...
int main(int argc, char **argv) {
  ...
  std::shared_ptr<rclcpp::Node> node =
      rclcpp::Node::make_shared("get_grid_map_client");
  ...
  if (rclcpp::spin_until_future_complete(node, result) ==
    ...
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Grid map info: %s",
                response->map.header.frame_id.c_str());
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Grid map resolution: %f",
