 ...
 * It is intentional that if raw maps come in faster than the post processing
 * works some raw maps are skipped. There is a minimal critical section updating
 * the availableServices_ structure. Workers operate on copies of the raw
...
class PostprocessorPool {
 public:
   ...
   * @param nodeHandle The node handle used to configure the task to run and to
   ...
   * @brief Destructor.
   *
   * Requests all postprocessing pipelines to stop and joins their threads until
   * they're done.
   */
  ~PostprocessorPool();

  /**
   * @brief Starts a task on a thread from the thread pool if there are some
   * available.
   ...
   * @return True if someone listens to the topic that the managed
   * postprocessor_ publishes to.
   */
 ...
 private:
   ...
   * @param serviceIndex The index of the thread / service that will process
   * this task.
   */
  ...
  //! Container holding the service ids which have corresponding threads. The
  //! only object that is used in a mutual exclusive manner and must be
  //! protected by availableServicesMutex_.
