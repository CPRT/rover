...
constexpr size_t kHeaderSize = 2 + (10 * sizeof(double)) + (2 * sizeof(uint16_t));
...
class GridmapDecompressor : public rclcpp::Node {
 public:
  explicit GridmapDecompressor(const rclcpp::NodeOptions &options = rclcpp::NodeOptions())
    ...
    pub_ = this->create_publisher<grid_map_msgs::msg::GridMap>("/viz/traversability_map", rviz_qos);
    sub_ = this->create_subscription<std_msgs::msg::UInt8MultiArray>(
        "/telemetry/gridmap_compressed", radio_qos,
        std::bind(&GridmapDecompressor::mapCallback, this, std::placeholders::_1));
  }
 ...
 private:
  void mapCallback(const std_msgs::msg::UInt8MultiArray::SharedPtr msg) {
    try {
      ...
      if (raw_payload.size() < kHeaderSize) {
        RCLCPP_WARN(this->get_logger(), "Dropped gridmap packet: payload smaller than header.");
        return;
      }
      ...
      const uint16_t outer_start = ReadLittleEndian<uint16_t>(raw_payload, offset);
      offset += sizeof(uint16_t);
      const uint16_t inner_start = ReadLittleEndian<uint16_t>(raw_payload, offset);
      offset += sizeof(uint16_t);

      if (raw_payload.size() < offset + frame_len) {
        RCLCPP_WARN(this->get_logger(), "Dropped gridmap packet: truncated frame_id bytes.");
        return;
      }

      const std::string frame_id(
          reinterpret_cast<const char *>(raw_payload.data() + offset), frame_len);
      offset += frame_len;

      const size_t remaining_bytes = raw_payload.size() - offset;
      if (remaining_bytes % 3 != 0) {
        RCLCPP_WARN(this->get_logger(), "Dropped gridmap packet: payload alignment is invalid.");
        return;
      }
      ...
      if (remaining_bytes < elev_bytes_len) {
        RCLCPP_WARN(this->get_logger(), "Dropped gridmap packet: elevation bytes missing.");
        return;
      }
      ...
      for (size_t i = 0; i < num_cells; ++i) {
        int16_t elev_val;
        std::memcpy(&elev_val, elev_bytes + i * sizeof(int16_t), sizeof(int16_t));
#if __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
  ...
  grid_msg.header.stamp.nanosec = static_cast<uint32_t>(now_ns % 1000000000LL);
      ...
      int size_x = std::max(1, static_cast<int>(std::round(length_x / resolution)));
      int size_y = std::max(1, static_cast<int>(std::round(length_y / resolution)));

      if (static_cast<size_t>(size_x) * static_cast<size_t>(size_y) != num_cells) {
        RCLCPP_WARN(this->get_logger(),
                    "Gridmap dimensions do not match payload size (%zu cells vs %d x %d).",
