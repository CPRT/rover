  ...
  void arm::arm_control(std::shared_ptr<sensor_msgs::msg::Joy> joystickMsg) {
    ...
    RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000, "Current Mode: %d", mode);
    ...
    if (mode == 1) {
      ...
      check_preset(joystickMsg);
    }
    last_joy_msg_ = joystickMsg;
  }
  ...
  void arm::arm_manual(std::shared_ptr<sensor_msgs::msg::Joy> joystickMsg) {
    ...
    joint_msg_.velocities = std::vector<double>{
  ...
  void arm::check_preset(std::shared_ptr<sensor_msgs::msg::Joy> joystickMsg){
      // Should only work if arm is not already moving
      // Check which goal pose is pressed

    if (is_moving_) return;

    mode_button_value = joystickMsg->buttons[kHomeButton];

    if (button_pressed(kHomeButton, joystickMsg)){
      request_position("saves.pose1");

    } else if (button_pressed(kStowButton, joystickMsg)){
      request_position("saves.pose2"); 
    }
  }

  bool arm::button_pressed(int index, const sensor_msgs::msg::Joy::SharedPtr& current){
    if (!last_joy_msg_){
      return false;
    }
    return (current->buttons[index] == 1 && last_joy_msg_->buttons[index] == 0);
  }
  ...
  void arm::request_position(std::string pose_id) {
    
    
    geometry_msgs::msg::Pose destination;

    positions.find(pose_id);
  ...
  void arm::loadParameters() {
    ...
    auto result = this->get_node_parameters_interface()->list_parameters({"saves"}, 0);

    for (auto &name : result.names) {

      positions[name] = this->get_parameter(name).as_double_array();
